...
clear all
clc
net_path = 'E:\dynamic_network_evolving_link_prediction\dynamic_networks\ia-enron-employees\ia-enron-employees.txt';
net_info = dlmread(net_path);

% method1: split the net into train and test by time
all_time = net_info(end, 4) - net_info(1,4);
train_sets = cell(1,9);
count = 1;
pre_line_num = 0;
N = 151;
start_time = net_info(1,4);
for i = 1:9
    end_time = net_info(pre_line_num+1, 4) + ceil(all_time * 0.1);
    temp = abs(end_time-net_info(:,4));
